...
public enum CommandSugarEnum {
  ...
  CREATEPYTHONENV(
      "createPythonEnv",
      "run command as PythonEnvExt.`{}` where condaFile=\"{}\" and command=\"create\""),
  REMOVEPYTHONENV(
      "removePythonEnv",
      "run command as PythonEnvExt.`{}` where condaFile=\"{}\" and command=\"remove\""),
  CREATEPYTHONENVFROMFILE(
      "createPythonEnvFromFile",
      "run command as PythonEnvExt.`{}` where condaYamlFilePath=\"${HOME}/{}\" and command=\"create\""),
  REMOVEPYTHONENVFROMFILE(
      "removePythonEnvFromFile",
      "run command as PythonEnvExt.`{}` where condaYamlFilePath=\"${HOME}/{}\" and command=\"remove\""),
  ...
  SAVEUPLOADFILETOHOME(
      "saveUploadFileToHome", "run command as DownloadExt.`` where from=\"{}\" and to=\"{}\""),
  WITHWARTERMARK(
      "withWartermark",
      "register WaterMarkInPlace.`` where inputTable=\"{}\" and eventTimeCol=\"{}\" and delayThreshold=\"{}\""),
  ...
  PYTHON(
      "python",
      "run command as PythonCommand.`` where parameters='''{:all}''' as {-1:next(named,uuid())}"),
  RAY(
      "ray",
      "run command as Ray.`` where inputTable='''{1}''' and code='''{2}''' and outputTable=\"{4}\" as {4:uuid()}"),
  ...
  CALLBACK(
      "callback",
      "run command as  MLSQLEventCommand.`` where eventName=\"{3}\" and handleHttpUrl=\"{1}\" and method=\"{0}\""),
  ...
  public static Map<String, String> toMap() {
    final Map<String, String> map =
        Arrays.stream(CommandSugarEnum.values())
    ...
    commands.forEach(
        (k, v) -> {
          ...
          if (value.contains("run") && value.contains("where") && value.contains("as")) {
            ...
            String syntax =
                String.format("run command as %s.`` where parameters='''{:all}'''", value);
